...
class OrderNew(BaseModel):
    ...
    required_meta: Optional[List[StrictStr]] = Field(default=None, description="Перечень метаданных, которые необходимо добавить в сборочное задание.  <br> На данный момент обязательным к добавлению является только UIN, при его наличии в перечне.              ", alias="requiredMeta")
    ...
    delivery_type: Optional[StrictStr] = Field(default=None, description="<dl> <dt>Тип доставки:</dt> <dd>fbs - доставка на склад Wildberries</dd> <dd>dbs - доставка силами продавца</dd> <dd>edbs - экспресс-доставка силами продавца</dd> <dd>wbgo - доставка курьером WB</dd> </dl> ", alias="deliveryType")
    ...
    __properties: ClassVar[List[str]] = ["id", "rid", "createdAt", "warehouseId", "offices", "address", "requiredMeta", "user", "skus", "price", "convertedPrice", "currencyCode", "convertedCurrencyCode", "orderUid", "deliveryType", "nmId", "chrtId", "article", "colorCode", "cargoType"]
    ...
    def delivery_type_validate_enum(cls, value):
        ...
        if value not in set(['dbs', 'fbs', 'edbs', 'wbgo']):
            raise ValueError("must be one of enum values ('dbs', 'fbs', 'edbs', 'wbgo')")
        return value

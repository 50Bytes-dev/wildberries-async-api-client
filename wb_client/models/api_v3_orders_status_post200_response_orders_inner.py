...
class ApiV3OrdersStatusPost200ResponseOrdersInner(BaseModel):
    ...
    """ # noqa: E501
    id: Optional[StrictInt] = Field(default=None, description="Идентификатор сборочного задания")
    supplier_status: Optional[StrictStr] = Field(default=None, description="Статус сборочного задания продавца (устанавливается продавцом)", alias="supplierStatus")
    ...
    def wb_status_validate_enum(cls, value):
        ...
        if value not in set(['waiting', 'sorted', 'sold', 'canceled', 'canceled_by_client']):
            raise ValueError("must be one of enum values ('waiting', 'sorted', 'sold', 'canceled', 'canceled_by_client')")
        return value

...
async def get_apiv3passesoffices(api_config_override: Optional[APIConfig] = None) -> List[PassOffice]:
    api_config = api_config_override if api_config_override else APIConfig()

    base_path = api_config.base_path or "https://marketplace-api.wildberries.ru"
    path = f"/api/v3/passes/offices"
    headers = {
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        async with session.request(
            ...
            return [PassOffice(**item) for item in response]


async def get_apiv3passes(api_config_override: Optional[APIConfig] = None) -> List[Pass]:
    api_config = api_config_override if api_config_override else APIConfig()

    base_path = api_config.base_path or "https://marketplace-api.wildberries.ru"
    path = f"/api/v3/passes"
    headers = {
        "Content-Type": "application/json",
        "Accept": "application/json",
        "Authorization": f"{ api_config.access_token }",
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        async with session.request(
            ...
            return [Pass(**item) for item in response]


async def post_apiv3passes(
    data: Dict[str, Any], api_config_override: Optional[APIConfig] = None
) -> ApiV3PassesPostResponse:
    api_config = api_config_override if api_config_override else APIConfig()
